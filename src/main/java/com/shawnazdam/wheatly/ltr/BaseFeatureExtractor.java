   ...
   */
    public void printFeatureForAllDocs(PrintStream out) throws IOException {
      ...
      for (int docId = 0; docId < reader.maxDoc(); docId ++) {
        ...
        out.flush();
        LOG.debug(String.format("Completed computing feature similarityPair for doc %d", docId));
      }
    }

    /**
     * Prints feature similarityPair wrt to the qrels, one vector per qrel
     * @param out
     * @throws IOException
     */
    public void printFeatures(PrintStream out) throws IOException {
      ...
      for (String qid : this.qrels.getQids()) {
        ...
        for (Map.Entry<String, Integer> entry : this.qrels.getDocMap(qid).entrySet()) {
          ...
          if (terms == null) {
            LOG.debug(String.format("No term similarityPair found for doc %s, qid %s", docId, qid));
            continue;
          }
